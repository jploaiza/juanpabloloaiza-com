  ...
  return (
    <main className="min-h-screen bg-[#020617] pt-32">
      ...
      >
        ...
        <h1 className="text-5xl md:text-6xl text-white mt-4 mb-6 font-cinzel font-bold">Blog</h1>
      ...
      >
        <div className="grid md:grid-cols-2 gap-8">
          {posts.map((post) => (
            <motion.div key={post.id} variants={itemVariants} className="group relative overflow-hidden cursor-pointer">
              <Link href={`/blog/${post.slug}`}>
                <div className="relative bg-[#0f172a] border-2 border-[#C5A059]/30 hover:border-[#C5A059]/60 transition duration-300 overflow-hidden">
                  ...
                  <div className="p-6">
                    ...
                    <p className="text-gray-300 text-sm leading-relaxed font-crimson mb-4 line-clamp-2">{post.excerpt}</p>
      ...
      >
        <div className="flex items-center justify-center gap-4">
          {page > 1 && (
            <Link href={`/blog?page=${page - 1}`} className="px-4 py-2 border border-[#C5A059]/30 hover:border-[#C5A059] text-[#C5A059] hover:bg-[#C5A059]/10 transition uppercase text-xs tracking-widest font-semibold">
              ← Anterior
            </Link>
          )}
